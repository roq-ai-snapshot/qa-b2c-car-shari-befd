...
import { getBillings } from 'apiSdk/billings';
...
function DashboardPage() {
  ...
  const billingStat = useSWR<PaginatedInterface<BillingInterface>>(
    () => `/billings`,
    () => getBillings({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Billings',
      result: billingStat?.data?.totalCount,
